...
function WeaponCasualitiesDetailsWindow.create(engine)
...
function WeaponCasualitiesDetailsWindow:bind_ctx(context)
  ...
  local weapon_casualities_details = engine.state.popups.weapon_casualities_details
  if (weapon_casualities_details) then
    ...
    local wi_ids = _.keys(weapon_casualities_details.participants)
    ...
    _.each(wi_ids, function(idx, wi_id)
      ...
      -- participants
      local p_label = create_image(tonumber(weapon_casualities_details.participants[wi_id]), AVAILABLE_COLOR)
      table.insert(line, {
        dy    = line[1].dy,
      ...
      -- casualities
      local c_label = create_image(tonumber(weapon_casualities_details.casualities[wi_id]), AVAILABLE_COLOR)
      table.insert(line, {
        dy    = line[1].dy,
    ...
    local x, y = weapon_casualities_details.position.x, weapon_casualities_details.position.y
    ...
    local mouse_click = function(event)
      engine.state.popups.weapon_casualities_details = nil
